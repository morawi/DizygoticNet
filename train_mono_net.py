...
from dizygotic_net import DizygoticNet
from color_count_dataset import ColorCountDataset
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    train_dataset = ColorCountDataset(mode = 'train', transforms = trn_transforms, 
                                      HPC_run = args.HPC_run)
    val_dataset = ColorCountDataset(mode = 'test', transforms = val_transforms, 
                                    HPC_run=args.HPC_run)
    ...
    if args.arch == 'effnet':  #  https://github.com/lukemelas/EfficientNet-PyTorch/issues/207
        model = DizygoticNet(num_classes)
    else:
        if args.pretrained:
...
def validate(val_loader, model, criterion, args):
    ...
    with torch.no_grad():
        ...
        for i, (image_p, image_n, colors, labels, num_colors) in enumerate(val_loader):
            # images, target = items # to be used with CIFAR data
